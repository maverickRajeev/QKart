...
class Search extends React.Component {
  constructor() {
    super();
    this.cartRef = React.createRef();  // reference for cart component
   ...
   */
  search = (text) => {
      let arr = this.products.filter(ele => {
      return (ele.name.toLowerCase().includes(text.toLowerCase()) || ele.category.toLowerCase().includes(text.toLowerCase()));
       });
  ...
  debounceSearch = (event) => {
      ...
      this.debounceTimeout = setTimeout(()=>{this.search(txt);},300); 
  };
