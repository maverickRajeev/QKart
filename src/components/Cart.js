...
export default class Cart extends React.Component {
  ...
  refreshCart = async () => {
    ...
    if (cart) {
      ...
      if(this.props.checkout === true && this.state.items.length === 0)
  ...
  getQuantityElement = (item) => {
    if(!this.props.checkout){
        return <InputNumber min={1} max={10} defaultValue={item.qty} onChange={(e) => this.pushToCart(item.productId, e ,false)}/>  
    }
    else {
  ...
  /**
   * method to redirect to the checkout page when checkout button is clicked
   */
  checkoutPage = () => {
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        {
          (!this.props.checkout) && ( 
          <Button icon = {<ShoppingCartOutlined />} onClick = {this.checkoutPage} style={{ background: "#FFCB0A", color: "white"}}><strong>&nbsp;Checkout</strong></Button>
          )  
        }
